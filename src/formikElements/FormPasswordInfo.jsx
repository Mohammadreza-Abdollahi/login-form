        ...
        <>
            <div>
                ...
                <div className="relative">
                    ...
                    <FastField name={name}>
                        {props=><input {...props.field} ref={inpRef} type={showPass ? 'text' : 'password'} id={name} placeholder={formik.errors[name] ? formik.errors[name] : placeholder} className={`${formik.errors[name] ? 'ring-red-500 focus:ring-red-500  placeholder:text-red-500': 'ring-label-border focus:ring-sub-2 placeholder:text-lg  placeholder:text-gray-400'} transition duration-200 block m-auto w-full rounded-xl outline-none px-3 py-3 text-heading ring-2 ring-inset ring-label-border focus:ring-2 text-xl pr-14`}/>}
                    </FastField>
                    {
                        formik.errors[name] ? (
